...
#include "SonicBirthRuntimeDesign.h"
...
#define CLASS_NAME SonicBirthRuntimeDesignEffect
	#include "SonicBirthRuntimeDesign.mm"
...
#define CLASS_NAME SonicBirthRuntimeDesignMidiEffect
	#include "SonicBirthRuntimeDesign.mm"
#undef SUPER_CLASS_NAME
#undef CLASS_NAME
...
#define CLASS_NAME SonicBirthRuntimeDesignMusicDevice
	#include "SonicBirthRuntimeDesign.mm"
#undef SUPER_CLASS_NAME
#undef CLASS_NAME
...
COMPONENT_ENTRY(SonicBirthRuntimeDesignEffect)
COMPONENT_ENTRY(SonicBirthRuntimeDesignMidiEffect)
COMPONENT_ENTRY(SonicBirthRuntimeDesignMusicDevice)
...
//------------------------------------------------------------------------------------------
CLASS_NAME::CLASS_NAME(AudioUnit component)
#ifdef MUSIC_DEVICE
...
// For this to compile, AUBase.h must be modified. Just change mInitNumInputEls to protected.
	...
	mSingleCircuit = YES; // for now, fixme
...
void CLASS_NAME::PostConstructor()
{
	...
	if ( Inputs().GetNumberOfElements() > 0 )
	{
		...
		inChannel = curInStreamFormat.mChannelsPerFrame;
	}
	
	const CAStreamBasicDescription curOutStreamFormat = GetStreamFormat(kAudioUnitScope_Output, (AudioUnitElement)0);
	outChannel = curOutStreamFormat.mChannelsPerFrame;

#ifndef MUSIC_DEVICE
	SetProcessesInPlace(inChannel == outChannel);
#endif
	...
	[c setNumberOfInputs:inChannel];
	[c setNumberOfOutputs:outChannel];
								...
								const CAStreamBasicDescription &inNewFormat)
{
	...
	int channels = inNewFormat.mChannelsPerFrame;
	if (channels <= 0)
			return false;
...
//------------------------------------------------------------------------------------------
void CLASS_NAME::argumentsChanged()
{
	// reregister arguments
	SBRootCircuit *c = mCircuits[0];
	[mListener registerEventsFromCircuit:c];

	// rebuild map
	mArgumentMap.clear();
	mSubArgumentMap.clear();
	mArgumentReverseMap.clear();
	
	LOG("argumentsChanged:\n")
	NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
...
/*
	...
	#warning "is it normal to have to call both methods ?"
...
*/
	...
	PropertyChanged(kAudioUnitProperty_ParameterInfo, kAudioUnitScope_Global, (AudioUnitElement)0);
	PropertyChanged(kAudioUnitProperty_ParameterValueStrings, kAudioUnitScope_Global, (AudioUnitElement)0);
}
...
SBRootCircuit* CLASS_NAME::CreateCircuit()
{
	...
	// set number of channels, etc, fixme
	[c setCanChangeNumberOfInputsOutputs:NO];
	[c setCanChangeHasSideChain:NO];
	
#ifndef MUSIC_DEVICE
	[c setHasSideChain:YES];
#endif
												...
												void *outData)
{
	...
	if (inScope == kAudioUnitScope_Global)
	{
		...
		{            
            ...
            {			
				NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
					NSString *identifier = @"com.sonicbirth.audiounit.design";
			...
			case kAudioUnitProperty_CocoaUI:
			{
				...
				CFStringRef className = CFStringCreateCopy(NULL, CFSTR("SBRuntimeDesignViewFactory"));
...
{
	LOG("GetParameterInfo paramId: %i\n", (int)inParameterID)
...
ComponentResult CLASS_NAME::SaveState( CFPropertyListRef * outData)
{
	...
	NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
		CFDictionarySetValue(dict, @"SonicBirthData", [c saveData]);
	[pool release];
...
ComponentResult CLASS_NAME::RestoreState( CFPropertyListRef	inData)
{
	...
	NSDictionary *data = (NSDictionary *)(CFDictionaryGetValue((CFDictionaryRef)inData, @"SonicBirthData"));
	if (!data)
		return kAudioUnitErr_InvalidPropertyValue;
...
ComponentResult CLASS_NAME::Initialize()
{
	...
	SBRootCircuit *cir = mCircuits[0];
	
	[cir setNumberOfInputs:inChannel];
	[cir setNumberOfOutputs:outChannel];
	...
	for (i = 0; i < c; i++)
		[mCircuits[i]	prepareForSamplingRate:mSampleRate
						...
						precision:[cir precision]
						interpolation:[cir interpolation]];
	...
	if (mSingleCircuit) mBuffersCount = [cir numberOfRealInputs];
	else mBuffersCount = c;
	
	if (mHasSideChain) mBuffersCount *= 2;
	/*if ([cir needsTempo])*/ mBuffersCount += 2;
												...
												UInt32 inFramesToProcess)
{
	...
	SBRootCircuit *cir = mCircuits[0];
	bool needsTempo = [cir needsTempo];
	bool hasFeedback = [cir hasFeedback];
	
	SBPrecision precision = cir->pPrecision;
	...
	while(frameToProcess > 0)
	{
		...
		if (hasFeedback && (todo > mMinFeedbackTime)) todo = mMinFeedbackTime;
		...
		{
			int max = 1 + ((needsTempo) ? 2 : 0) + ((mHasSideChain) ? 1 : 0);
			if (precision == kFloatPrecision)
			{
				for (j = 0; j < c; j++)
				{
