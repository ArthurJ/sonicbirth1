...
#define kRadius (mSliderHeight / 2.f)

#define kButtonMinMaxCourse ((float)(kButtonWidth))

#define kTextSpace (3)
#define kTextHeight (12)
#define kTextWidth (32)

@implementation SBSliderHorCell
...
- (void) drawContentAtPoint:(NSPoint)origin
{
	...
	if (mFrontImage)
	{
		float range = kButtonWidth - ogImageWidth(mFrontImage);
		pt.x += pos * range;
		pt.y += (kButtonHeight - ogImageHeight(mFrontImage)) / 2;
		ogDrawImage(mFrontImage, pt.x, pt.y);
	}
	else
	{
		float range = kButtonWidth - kRadius - kRadius;
		pt.x += pos * range;
...
- (BOOL) mouseDraggedX:(int)x Y:(int)y lastX:(int)lx lastY:(int)ly
{
	...
	cur = cur + (x - mStartPos)/kButtonMinMaxCourse * (max - min);
	if (cur < min) cur = min;
