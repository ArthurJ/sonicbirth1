...
static void privateCalcFunc(void *inObj, int count, int offset)
{
	...
	if (obj->mPrecision == kFloatPrecision)
	{
		...
		while(count--)
		{
			*rnd++ = (obj->mValue * phase) + (obj->mLastValue * (1.f - phase));
			phase += (*f++) * sr;
			if (phase > 1.f)
			{
				phase = fmodf(phase, 1.f);
				obj->mLastValue = obj->mValue;
				obj->mValue = (((double)random() * 2.) /  kRandomMax) - 1.;
			}
		}
		obj->mPhase = phase;
	}
	else if (obj->mPrecision == kDoublePrecision)
	{
		...
		while(count--)
		{
			*rnd++ = (obj->mValue * phase) + (obj->mLastValue * (1. - phase));
			phase += (*f++) * sr;
			if (phase > 1.)
			{
				phase = fmod(phase, 1.);
...
{
	return @"Generates random values between -1 and 1, changing the value at frequency f, interpolating between values.";
}
...
- (void) reset
{
	...
	mPhase = 0.;
	mLastValue = 0.;
	mValue = (((double)random() * 2.) /  kRandomMax) - 1.;
}
