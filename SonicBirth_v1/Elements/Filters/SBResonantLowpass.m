...
#define UPDATE_COEFF_FLOAT(f, r, sr, a0, a1, a2, b1, b2) \
	...
	{ \
		...
		if (f < 1.f) f = 1.f; \
		float c = 1.f / tanf((float)M_PI * f / sr); \
		float g = 1.f; \
		float k = c*c + r*c + g; \
		a0 = 1.f / (g*k); \
		a1 = a0+a0; \
...
@implementation SBResonantLowpass

+ (NSString*) name
{
	return @"Resonant lowpass";
}
...
{
	return	@"Resonant lowpass filter, 12/db octave (Butterworth), with variable cutoff frequency (clamped to [20, 20000]). "
